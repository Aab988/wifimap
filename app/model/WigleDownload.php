...
class WigleDownload extends Download implements \IDownload {
     ...
     */
    private function getAllRequestsInLatLngRange($lat_start,$lat_end,$lon_start,$lon_end) {
        return $this->database->table("wigle_request")
            ->select("lat_start,lat_end,lon_start,lon_end")
            ->where('lat_start >= ? OR lat_end <= ?', $lat_start,$lat_end)
            ->where('lon_start >= ? OR lon_end <= ?', $lon_start,$lon_end)
            ->where('processed','N')
            ->fetchAll();
    }
     ...
     * @param float $lat1 smallest latitude
     * @param float $lat2 biggest latitude
     * @param float $lon1 smallest longitude
     * @param float $lon2 bigges longitude
     * @return array
     */
    private function createMappingXY($data,$lat1,$lat2,$lon1,$lon2) {
    ...
    private function unmapArrayOfRectanglesFromIndexToLatLng($rectangles,$mappingX,$mappingY) {
        ...
        for($y = 0; $y < count($rectangles); $y++) {
            foreach($rectangles[$y] as $zero) {
                $unmapped[] = array("x"=> array("from"=>$mappingX[$zero["from"]],"to"=>$mappingX[$zero['to']]), "y" => array("from" => $mappingY[$y], "to" => $mappingY[$y+1]));
            }
        }
        return $unmapped;
    }
     ...
     * @return array
     */
    public function findNotInQueueRectsInLatLngRange($lat1,$lat2,$lon1,$lon2) {
        // sort lat and lng by size

        $sorted = CoordsUtils::sortCoordsLatLngBySize($lat1,$lat2,$lon1,$lon2);
        $lat_start = $sorted['lat_start']; $lat_end = $sorted['lat_end'];
        $lon_start = $sorted['lon_start']; $lon_end = $sorted['lon_end'];

        // get all requests in this range
        $data = $this->getAllRequestsInLatLngRange($lat_start,$lat_end,$lon_start,$lon_end);

        // create latitude, longitude XY mapping
        $mapping = $this->createMappingXY($data,$lat_start,$lat_end,$lon_start,$lon_end);
